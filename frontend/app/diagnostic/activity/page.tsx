...
export default function DiagnosticActivityPage() {
    ...
    const fetchActivities = async () => {
        try {
            setLoading(true);
            const res = await fetch(`${backendUrl}/api/diagnostic/activity/${session?.user?.providerId}?action=${filter}`);
    ...
    const getActivityTitle = (activity: any) => {
        switch (activity.action) {
            case 'upload':
                return `Uploaded: ${activity.details?.title || 'Test Result'}`;
            case 'request_access':
        ...
        <div className="min-h-screen bg-gray-50 flex">
            ...
            <aside className="w-64 bg-white h-screen shadow-md flex flex-col fixed">
                ...
                <nav className="flex-1 mt-6">
                    ...
                    <Link href="/diagnostic/activity" className="flex items-center px-6 py-3 bg-gray-100 text-gray-900 border-r-4 border-cyan-600">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
